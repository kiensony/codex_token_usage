...
def render_report(
    ...
    if output_format == "json":
        return render_json(rows, dataset, group_by)
    if output_format == "csv":
        return render_csv(rows, group_by)
    if output_format == "graph":
        return render_graph(rows, group_by, theme=theme, color_enabled=color_enabled)
    raise ValueError(f"unsupported format: {output_format}")


def render_table(rows: list[ReportRow], group_by: str) -> str:
    if group_by == "session":
        headers = [
            "session",
            "title",
    ...
    return table_from_rows(headers, body)


def render_json(rows: list[ReportRow], dataset: UsageDataset, group_by: str) -> str:
    payload = {
...
def render_csv(rows: list[ReportRow], group_by: str) -> str:
    output = io.StringIO()
    if group_by == "session":
        fieldnames = [
            "session",
    ...
    writer.writeheader()
    for row in rows:
        data = row_to_csv_dict(row, group_by)
...
def row_to_dict(row: ReportRow) -> dict[str, object]:
    return {
        "key": row.key,
...
def row_to_csv_dict(row: ReportRow, group_by: str) -> dict[str, object]:
    if group_by == "session":
        return {
            "session": row.key,
            "title": row.title or "",
